...
export default function BottomMenu({ navigation, currentRoute, onAddPress, onGoalsPress }) {
  ...
  return (
    <View style={[styles.bottomMenu, { paddingBottom: insets.bottom }]}> 
      {bottomIcons.map((item, idx) => {
        ...
        let iconStyle;
        if (idx === 2) {
          iconStyle = [styles.centerIcon, isActive ? { tintColor: '#00e6a0' } : { tintColor: '#1a234d' }];
        } else {
          iconStyle = [styles.bottomIcon, isActive ? { tintColor: '#00e6a0' } : { tintColor: '#fff' }];
        }
        return (
          <TouchableOpacity
            key={item.key}
            ...
            onPress={() => {
              ...
              } else if (idx === 2 && onAddPress) {
                onAddPress();
              } else if (idx === 3 && navigation) {
                navigation.navigate('Stats');
              } else if (navigation) {
                navigation.navigate(routeMap[idx]);
              }
            }}
            disabled={isActive}
          >
